...
pub mod config;
pub mod drt_dcdt_safe;

use config::Config;
use dharitri_sc_snippets::imports::*;
use drt_dcdt_safe::drt_dcdt_safe_interactor_main::DrtDcdtSafeInteract;
...
pub async fn drt_dcdt_safe_cli() {
    env_logger::init();
    ...
    let mut interact = DrtDcdtSafeInteract::new(config).await;
    match cmd.as_str() {
...
pub struct State {
    pub drt_dcdt_safe_address: Option<Bech32Address>,
...
impl State {
    ...
    pub fn set_drt_dcdt_safe_contract_address(&mut self, address: Bech32Address) {
        self.drt_dcdt_safe_address = Some(address);
    }
    ...
    pub fn current_drt_dcdt_safe_contract_address(&self) -> &Bech32Address {
        self.drt_dcdt_safe_address
            .as_ref()
