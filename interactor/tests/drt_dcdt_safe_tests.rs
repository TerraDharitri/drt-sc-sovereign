use common_test_setup::constants::{
    ...
    SOV_TOKEN, SOV_TO_DRT_TOKEN_STORAGE_KEY, TOKEN_TICKER,
};
use common_test_setup::RegisterTokenArgs;
use error_messages::{
    BANNED_ENDPOINT_NAME, GAS_LIMIT_TOO_HIGH, INVALID_TYPE, NOTHING_TO_TRANSFER,
    NO_DCDT_SAFE_ADDRESS, PAYMENT_DOES_NOT_COVER_FEE, TOO_MANY_TOKENS,
};
use header_verifier::OperationHashStatus;
use dharitri_sc_snippets::dharitri_sc_scenario::dharitri_chain_vm::crypto_functions::sha256;
use dharitri_sc_snippets::{hex, imports::*};
use proxies::fee_market_proxy::{FeeStruct, FeeType};
use rust_interact::config::Config;
use rust_interact::drt_dcdt_safe::drt_dcdt_safe_interactor_main::DrtDcdtSafeInteract;
use serial_test::serial;
use structs::aliases::PaymentsVec;
use structs::configs::DcdtSafeConfig;
use structs::operation::{Operation, OperationData, OperationDcdtPayment, TransferData};

// Test that deposit fails when there is nothing to transfer and fee is disabled
// Steps:
// 1. Deploy header verifier smart contract
// 2. Deploy drt-dcdt-safe smart contract
// 3. Deploy fee-market smart contract
// 4. Set fee-market address in drt-dcdt-safe smart contract
// 5. Unpause drt-dcdt-safe smart contract
// 6. Deposit tokens
#[tokio::test]
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn deposit_nothing_to_transfer_no_fee() {
    let mut chain_interactor = DrtDcdtSafeInteract::new(Config::chain_simulator_config()).await;

    chain_interactor
        .deploy_contracts(OptionalValue::Some(DcdtSafeConfig::default_config()), None)
        .await;
...
// 4. Set fee-market address in drt-dcdt-safe smart contract
// 5. Unpause drt-dcdt-safe smart contract
// 6. Create a payments vector consisting of 11 DcdtTokenPayments
// 7. Deposit tokens
#[tokio::test]
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
...
// Steps:
// 1. Create an DcdtSafeConfig with a banned endpoint
// 2. Deploy header verifier smart contract
// 3. Deploy drt-dcdt-safe smart contract
// 4. Deploy fee-market smart contract
// 5. Set fee-market address in drt-dcdt-safe smart contract
// 6. Unpause drt-dcdt-safe smart contract
...
async fn register_token_invalid_type_token() {
    ...
    let sov_token_id = TokenIdentifier::from_dcdt_bytes(SOV_TOKEN);
    let token_type = DcdtTokenType::Invalid;
    let token_display_name = "SOVEREIGN";
    let num_decimals = 18;
    let token_ticker = TOKEN_TICKER;
    let rewa_payment = BigUint::from(ISSUE_COST);
...
// 3. Create the token properties
// 4. Register the token
// 5. Check that the token ticker exists in sov-to-drt-token storage
#[tokio::test]
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn register_token_fungible_token() {
    ...
    let encoded_key = &hex::encode(SOV_TO_DRT_TOKEN_STORAGE_KEY);

    chain_interactor
        .check_account_storage(
            chain_interactor
                .state
                .current_drt_dcdt_safe_contract_address()
                .clone()
                .to_address(),
            encoded_key,
...
async fn register_token_dynamic_non_fungible_token() {
    ...
    let sov_token_id = TokenIdentifier::from_dcdt_bytes(SOV_TOKEN);
    let token_type = DcdtTokenType::DynamicNFT;
    let token_display_name = "SOVEREIGN";
    let num_decimals = 18;
    let token_ticker = TOKEN_TICKER;
    let rewa_payment = BigUint::from(ISSUE_COST);
...
// Test that execute operation fails when the dcdt safe address is not registered
// Steps:
// 1. Deploy header verifier smart contract
// 2. Deploy drt-dcdt-safe smart contract
// 3. Deploy fee-market smart contract
...
#[serial]
#[cfg_attr(not(feature = "chain-simulator-tests"), ignore)]
async fn execute_operation_no_dcdt_safe_registered() {
    let mut chain_interactor = DrtDcdtSafeInteract::new(Config::chain_simulator_config()).await;
...
// 4. Deploy drt-dcdt-safe smart contract
// 5. Deploy fee-market smart contract
// 6. Set fee-market address in drt-dcdt-safe smart contract
// 7. Unpause drt-dcdt-safe smart contract
// 8. Deploy testing smart contract
// 9. Create operation
// 10. Deposit and check logs
// 11. Set drt-dcdt-safe address in header verifier smart contract
...
async fn execute_operation_success_no_fee() {
    ...
    payment_vec.push(DcdtTokenPayment {
        token_identifier: TokenIdentifier::from_dcdt_bytes(FIRST_TEST_TOKEN),
        token_nonce: 0,
    ...
    chain_interactor
        .set_dcdt_safe_address_in_header_verifier()
        .await;
...
// 7. Deploy testing smart contract
// 8. Create operation
// 9. Set drt-dcdt-safe address in header verifier smart contract
