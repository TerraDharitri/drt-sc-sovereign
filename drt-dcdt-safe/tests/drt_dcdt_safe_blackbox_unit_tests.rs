use common_test_setup::constants::{
    DCDT_SAFE_ADDRESS, FEE_MARKET_ADDRESS, FEE_TOKEN, FIRST_TEST_TOKEN, HEADER_VERIFIER_ADDRESS,
...
use error_messages::{
    ...
    NOTHING_TO_TRANSFER, NO_DCDT_SAFE_ADDRESS, PAYMENT_DOES_NOT_COVER_FEE, TOKEN_ID_IS_NOT_TRUSTED,
...
use dharitri_sc::{
    ...
    types::{
        BigUint, DcdtTokenData, DcdtTokenPayment, DcdtTokenType, ManagedBuffer, ManagedVec,
        TestTokenIdentifier, TokenIdentifier,
    },
};
use dharitri_sc_scenario::dharitri_chain_vm::crypto_functions::sha256;
use dharitri_sc_scenario::{api::StaticApi, ScenarioTxWhitebox};
use drt_dcdt_safe::bridging_mechanism::{BridgingMechanism, TRUSTED_TOKEN_IDS};
use drt_dcdt_safe_blackbox_setup::DrtDcdtSafeTestState;
...
use structs::{
    ...
    operation::{Operation, OperationData, OperationDcdtPayment},
};
mod drt_dcdt_safe_blackbox_setup;

#[test]
fn deploy() {
    let mut state = DrtDcdtSafeTestState::new();

    state.deploy_contract(
        HEADER_VERIFIER_ADDRESS,
        OptionalValue::Some(DcdtSafeConfig::default_config()),
    );
}
...
fn register_token_invalid_type_with_prefix() {
    let mut state = DrtDcdtSafeTestState::new();
    let config = DcdtSafeConfig::default_config();
    ...
    state.register_token(register_token_args, rewa_payment, Some(INVALID_TYPE));

    state
        .common_setup
        .check_dharitri_to_sovereign_token_id_mapper_is_empty(FIRST_TEST_TOKEN);
}
...
fn register_token_fungible_token() {
    ...
    state.register_token(register_token_args, rewa_payment, None);
}
...
fn register_token_nonfungible_token() {
    ...
    let sov_token_id = TestTokenIdentifier::new(FIRST_TEST_TOKEN);
    let token_type = DcdtTokenType::NonFungible;
    let token_display_name = "TokenOne";
    let num_decimals = 0;
    let token_ticker = FIRST_TEST_TOKEN;
    let rewa_payment = BigUint::from(DEFAULT_ISSUE_COST);
...
fn deposit_too_many_tokens() {
    ...
    state.common_setup.deploy_fee_market(None);
    state.set_fee_market_address(FEE_MARKET_ADDRESS);
    let dcdt_token_payment = DcdtTokenPayment::<StaticApi>::new(
        TokenIdentifier::from(FIRST_TEST_TOKEN),
        0,
        BigUint::default(),
    );

    let payments_vec = PaymentsVec::from(vec![dcdt_token_payment; 11]);
...
fn deposit_no_transfer_data() {
    ...
    let dcdt_token_payment_one = DcdtTokenPayment::<StaticApi>::new(
        TokenIdentifier::from(FIRST_TEST_TOKEN),
        0,
        BigUint::from(100u64),
    );

    let dcdt_token_payment_two = DcdtTokenPayment::<StaticApi>::new(
        TokenIdentifier::from(SECOND_TEST_TOKEN),
        0,
        BigUint::from(100u64),
    );

    let payments_vec = PaymentsVec::from(vec![dcdt_token_payment_one, dcdt_token_payment_two]);
...
fn deposit_gas_limit_too_high() {
    ...
    let config = DcdtSafeConfig::new(ManagedVec::new(), ManagedVec::new(), 1, ManagedVec::new());
    state.deploy_contract(HEADER_VERIFIER_ADDRESS, OptionalValue::Some(config));
    state.common_setup.deploy_fee_market(None);
    state.set_fee_market_address(FEE_MARKET_ADDRESS);
...
/// Steps for this test:
/// 1. Deploy the Drt-DCDT-Safe smart contract
/// 3. Deploy the Testing smart contract
/// 6. Create the DCDT token payments
...
fn deposit_transfer_data_only_with_fee() {
    ...
    DcdtTokenPayment::<StaticApi>::new(TokenIdentifier::from(FEE_TOKEN), 0, fee_amount.clone());
    let fee_payment =
        DcdtTokenPayment::<StaticApi>::new(TokenIdentifier::from(FEE_TOKEN), 0, fee_amount.clone());
...
fn deposit_fee_enabled() {
    ...
    let payments_vec = PaymentsVec::from(vec![
        fee_payment,
        dcdt_token_payment_one.clone(),
        dcdt_token_payment_two.clone(),
    ]);
    ...
    let expected_amount_token_one =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_one.amount;
    ...
    let expected_amount_token_two =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_two.amount;
    ...
    state
        ...
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(TokenIdentifier::from(FEE_TOKEN), expected_amount_token_fee);
}
...
fn deposit_payment_doesnt_cover_fee() {
    ...
    state
        .common_setup
        .check_dharitri_to_sovereign_token_id_mapper_is_empty(SECOND_TEST_TOKEN);
}
...
fn deposit_success_burn_mechanism() {
    ...
    let dcdt_token_payment_trusted_token = DcdtTokenPayment::<StaticApi>::new(
    ...
    let payments_vec = PaymentsVec::from(vec![
        dcdt_token_payment_trusted_token.clone(),
        dcdt_token_payment_two.clone(),
    ]);
    ...
    state
        .common_setup
        .check_dharitri_to_sovereign_token_id_mapper_is_empty(TRUSTED_TOKEN_IDS[0]);

    state.common_setup.check_sc_dcdt_balance(
        vec![
        ...
        DCDT_SAFE_ADDRESS.to_managed_address(),
        drt_dcdt_safe::contract_obj,
    )
}
...
/// Test that execute operation fails when the Drt-DCDT-Safe address is not set in Header-Verifier contract
/// Steps:
/// 1. Deploy the Drt-DCDT-Safe smart contract
...
#[test]
fn execute_operation_no_dcdt_safe_registered() {
    let mut state = DrtDcdtSafeTestState::new();
    let config = OptionalValue::Some(DcdtSafeConfig::default_config());
    state.deploy_contract(HEADER_VERIFIER_ADDRESS, config);

    let payment = OperationDcdtPayment::new(
        TokenIdentifier::from(FIRST_TEST_TOKEN),
        0,
        DcdtTokenData::default(),
    );
...
/// 6. Set the Drt-DCDT-Safe address in the Header-Verifier smart contract
...
fn execute_operation_success() {
    ...
    let token_data = DcdtTokenData {
        amount: BigUint::from(100u64),
        ..Default::default()
    };

    let payment = OperationDcdtPayment::new(TokenIdentifier::from(FIRST_TEST_TOKEN), 0, token_data);
    ...
    state.common_setup.deploy_header_verifier();
    state.common_setup.deploy_testing_sc();
    state.set_dcdt_safe_address_in_header_verifier(DCDT_SAFE_ADDRESS);
...
/// 7. Set the Drt-DCDT-Safe address in the Header-Verifier smart contract
...
fn execute_operation_with_native_token_success() {
    ...
    state.register_native_token(FIRST_TEST_TOKEN, token_display_name, rewa_payment, None);
...
/// Steps for this test:
/// 1. Deploy the Drt-DCDT-Safe SC with roles for the trusted token
/// 2. Create the `operation`
...
/// 10. Check the balances for the owner, Drt-DCDT-Safe and Testing SC
#[test]
fn execute_operation_burn_mechanism_without_deposit_cannot_subtract() {
    ...
    let payment =
        OperationDcdtPayment::new(TokenIdentifier::from(TRUSTED_TOKEN_IDS[0]), 0, token_data);
    ...
    state.register_native_token(TRUSTED_TOKEN_IDS[0], token_display_name, rewa_payment, None);
    state.set_token_burn_mechanism(TRUSTED_TOKEN_IDS[0], None);
...
/// 2. Create the `operation`
/// 3. Deploy the needed smart contract (Header-Verifier, Fee-Market with no fee and Testing SC)
/// 4. Set the Fee-Market address in Drt-DCDT-Safe and Header-Verifier
...
/// 11. Check the balances for the owner, Drt-DCDT-Safe and Testing SC
...
/// 3. Set the Fee-Market address in Drt-DCDT-Safe and Header-Verifier
/// 4. Deposit the `deposit_payment` to the `USER`
/// 5. Check for logs and dcdt balance
/// 6. Switch the bridging mechanism to Burn&Mint for the trusted token
/// 7. Check for `deposited_tokens_amount` mapper and dcdt balance
/// 8. Create the first `operation`
/// 9. Register the `operation`
/// 10. Execute the `operation`
/// 11. Check for `deposited_tokens_amount` mapper and dcdt balance
/// 12. Second deposit of `deposit_payment` to the `USER`
/// 13. Check for logs, `deposited_tokens_amount` mapper and dcdt balance
/// 14. Set bridging mechanism back to Lock&Send
/// 15. Check `deposited_tokens_amount` mapper and dcdt balance
/// 16. Create the second `operation`
/// 17. Register the `operation`
/// 18. Execute the `operation`
/// 19. Check for `deposited_tokens_amount` mapper and dcdt balance
/// 12. Third deposit of `deposit_payment` to the `USER`
/// 19. Check for logs, `deposited_tokens_amount` mapper and dcdt balance
#[test]
fn deposit_execute_switch_mechanism() {
    ...
    let deposit_trusted_token_payment_token_data = DcdtTokenData {
        amount: BigUint::from(deposited_trusted_token_payment_amount),
        ..Default::default()
    };
    let deposit_trusted_token_payment = OperationDcdtPayment::new(
    ...
    state
        .common_setup
        .world
        .check_account(DCDT_SAFE_ADDRESS)
        .dcdt_balance(TestTokenIdentifier::new(trusted_token_id), 1000);
    ...
    let execute_trusted_token_payment_token_data = DcdtTokenData {
        amount: BigUint::from(execute_trusted_token_payment_amount),
        ..Default::default()
    };
    let execute_trusted_token_payment = OperationDcdtPayment::new(
        TokenIdentifier::from(trusted_token_id),
        0,
...
/// Steps for this test:
/// 1. Deploy the Drt-DCDT-Safe SC with the default config
...
fn set_token_burn_mechanism_no_roles() {
    ...
    state.set_token_burn_mechanism("WREWA", Some(MINT_AND_BURN_ROLES_NOT_FOUND));
}
...
fn set_token_lock_mechanism_token_from_sovereign() {
    ...
    state
        ...
        .to(DCDT_SAFE_ADDRESS)
        .whitebox(drt_dcdt_safe::contract_obj, |sc| {
            sc.dharitri_to_sovereign_token_id_mapper(&TokenIdentifier::from(
                TRUSTED_TOKEN_IDS[0],
            ))
