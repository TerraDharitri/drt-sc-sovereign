...
pub struct FinalPayment<M: ManagedTypeApi> {
    pub fee: DcdtTokenPayment<M>,
    pub remaining_tokens: DcdtTokenPayment<M>,
}
    ...
    + utils::UtilsModule
{
        ...
        for token_id in self.tokens_for_fees().iter() {
            ...
            for pair in &pairs {
                ...
                if amount_to_send > 0 {
                    ...
                    self.tx()
                        .to(&pair.address)
                        .payment(DcdtTokenPayment::new(token_id.clone(), 0, amount_to_send))
                        .transfer();
                }
            }
    ...
    fn subtract_fee(
        ...
        let payment = self.call_value().single_dcdt().clone();

        if !self.is_fee_enabled() || self.users_whitelist().contains(&original_caller) {
            self.tx().to(&caller).payment(&payment).transfer();

            return FinalPayment {
                fee: DcdtTokenPayment::new(payment.token_identifier.clone(), 0, BigUint::zero()),
    ...
    fn subtract_fee_same_token(
        ...
        FinalPayment {
            fee: DcdtTokenPayment::new(payment.token_identifier.clone(), 0, total_fee),
            remaining_tokens: payment,
        }
    }
    ...
    fn subtract_fee_any_token(
        ...
        args.payment = DcdtTokenPayment::new(
        ...
        // User pays 100 REWA, which by asking the pair you found out is 2000 RIDE
        // Then the cost for the user is (1500 RIDE * 100 REWA / 2000 RIDE = 75 REWA)
        ...
        FinalPayment {
            ...
            remaining_tokens: DcdtTokenPayment::new(
