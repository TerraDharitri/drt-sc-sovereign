...
pub trait BridgingMechanism:
    cross_chain::storage::CrossChainStorage + dharitri_sc_modules::only_admin::OnlyAdminModule
{
    #[only_admin]
    #[endpoint(setTokenBurnMechanism)]
    fn set_token_burn_mechanism(&self, token_id: TokenIdentifier) {
        let token_dcdt_roles = self.blockchain().get_dcdt_local_roles(&token_id);

        require!(
            token_dcdt_roles.contains(DcdtLocalRoleFlags::MINT)
                && token_dcdt_roles.contains(DcdtLocalRoleFlags::BURN),
        ...
        if self
            .dharitri_to_sovereign_token_id_mapper(&token_id)
            .is_empty()
        {
        ...
        let sc_balance = self
            .blockchain()
            .get_sc_balance(&RewaOrDcdtTokenIdentifier::dcdt(token_id.clone()), 0);

        if sc_balance != 0 {
            self.tx()
                .to(ToSelf)
                .typed(UserBuiltinProxy)
                .dcdt_local_burn(&token_id, 0, &sc_balance)
                .sync_call();
    ...
    fn set_token_lock_mechanism(&self, token_id: TokenIdentifier) {
        require!(
            self.dharitri_to_sovereign_token_id_mapper(&token_id)
                .is_empty(),
        ...
        if deposited_amount != 0 {
            self.tx()
                .to(ToSelf)
                .typed(UserBuiltinProxy)
                .dcdt_local_mint(&token_id, 0, &deposited_amount)
                .sync_call();
