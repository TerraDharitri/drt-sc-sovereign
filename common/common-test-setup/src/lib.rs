...
pub mod constants;
use constants::{
    CHAIN_CONFIG_ADDRESS, CHAIN_CONFIG_CODE_PATH, DCDT_SAFE_ADDRESS, FEE_MARKET_ADDRESS,
...
pub struct AccountSetup<'a> {
    ...
    pub dcdt_balances: Option<Vec<(TestTokenIdentifier<'a>, BigUint<StaticApi>)>>,
    pub rewa_balance: Option<BigUint<StaticApi>>,
}
...
impl BaseSetup {
    ...
    pub fn new(account_setups: Vec<AccountSetup>) -> Self {
        ...
        for acc in account_setups {
            ...
            if let Some(dcdt_balances) = acc.dcdt_balances {
                for (token_id, amount) in dcdt_balances {
                    acc_builder = acc_builder.dcdt_balance(token_id, amount);
                }
            }

            if let Some(balance) = acc.rewa_balance {
    ...
    // TODO: Add a better check balance for dcdt function after check storage is fixed
    pub fn check_sc_dcdt_balance<ContractObj>(
        &mut self,
    ...
    pub fn check_dharitri_to_sovereign_token_id_mapper_is_empty(&mut self, token_name: &str) {
        self.world
            .query()
            .to(DCDT_SAFE_ADDRESS)
            .whitebox(drt_dcdt_safe::contract_obj, |sc| {
                assert!(sc
                    .dharitri_to_sovereign_token_id_mapper(
