    ...
    crate::storage::CrossChainStorage + crate::execute_common::ExecuteCommonModule + utils::UtilsModule
{
    fn match_fee_payment(
        ...
        fees_payment: &OptionalValue<DcdtTokenPayment<Self::Api>>,
    ...
    fn burn_sovereign_token(&self, payment: &DcdtTokenPayment<Self::Api>) {
        self.tx()
            .to(ToSelf)
    ...
    fn get_event_payment_token_data(
        ...
        let mut current_token_data = self.blockchain().get_dcdt_token_data(
            current_sc_address,
            &payment.token_identifier,
            payment.token_nonce,
        );
    ...
    fn refund_tokens(
        ...
        refundable_payments: ManagedVec<DcdtTokenPayment<Self::Api>>,
    ) {
        self.tx()
            .to(caller)
            .multi_dcdt(refundable_payments)
    ...
    fn burn_mainchain_token(
        &self,
        payment: DcdtTokenPayment<Self::Api>,
        payment_token_type: &DcdtTokenType,
        ...
        if payment.token_nonce > 0 {
            sov_token_nonce = self
                .dharitri_to_sovereign_dcdt_info_mapper(
                    &payment.token_identifier,
                    payment.token_nonce,
                )
                .get()
            ...
            if self.is_nft(payment_token_type) {
                self.clear_drt_to_sov_dcdt_info_mapper(
                    &payment.token_identifier,
                    payment.token_nonce,
                );

                self.clear_sov_to_drt_dcdt_info_mapper(sov_token_id, sov_token_nonce);
            }
        }
    ...
    #[inline]
    fn clear_drt_to_sov_dcdt_info_mapper(&self, id: &TokenIdentifier, nonce: u64) {
        self.dharitri_to_sovereign_dcdt_info_mapper(id, nonce)
            .take();
    }

    #[inline]
    fn clear_sov_to_drt_dcdt_info_mapper(&self, id: &TokenIdentifier, nonce: u64) {
    ...
    fn is_token_whitelist_empty(&self) -> bool {
        self.dcdt_safe_config().get().token_whitelist.is_empty()
    }
    ...
    fn require_gas_limit_under_limit(&self, gas_limit: GasLimit) {
        require!(
            gas_limit <= self.dcdt_safe_config().get().max_tx_gas_limit,
            GAS_LIMIT_TOO_HIGH
        );
    }
}
