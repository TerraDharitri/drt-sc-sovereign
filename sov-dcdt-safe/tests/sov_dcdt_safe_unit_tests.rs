use common_test_setup::constants::{
    DCDT_SAFE_ADDRESS, FEE_MARKET_ADDRESS, FEE_TOKEN, FIRST_TEST_TOKEN, ONE_HUNDRED_MILLION,
    ONE_HUNDRED_THOUSAND, OWNER_ADDRESS, SECOND_TEST_TOKEN, USER,
};
use error_messages::NOTHING_TO_TRANSFER;
use dharitri_sc::{
    imports::{MultiValue3, OptionalValue},
    types::{
        BigUint, DcdtTokenPayment, ManagedBuffer, ManagedVec, MultiValueEncoded,
        TestTokenIdentifier, TokenIdentifier,
    },
};
use dharitri_sc_scenario::api::StaticApi;
use proxies::fee_market_proxy::{FeeStruct, FeeType};
use sov_dcdt_safe_setup::SovDcdtSafeTestState;
use structs::{aliases::PaymentsVec, configs::DcdtSafeConfig};

mod sov_dcdt_safe_setup;

#[test]
fn deploy() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract(
        FEE_MARKET_ADDRESS,
        OptionalValue::Some(DcdtSafeConfig::default_config()),
    );
}

/// Test the deposit function without fee and without transfer data.
/// Steps:
/// 1. Deploy the Sov-DCDT-Safe smart contract with roles.
/// 2. Deploy the Fee-Market smart contract.
/// 3. Deploy the Testing smart contract.
/// 4. Set the Fee-Market address.
/// 5. Create two DCDT token payments.
/// 6. Create a payments vector with the two DCDT token payments.
/// 7. Call the deposit function with the payments vector.
/// 8. Check the logs for the deposit function.
/// 9. Check the DCDT balance of the addresses
#[test]
fn deposit_no_fee_no_transfer_data() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();

    state.common_setup.deploy_fee_market(None);
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    let test_token_one_identifier = TestTokenIdentifier::new(FIRST_TEST_TOKEN);
    let test_token_two_identifier = TestTokenIdentifier::new(SECOND_TEST_TOKEN);

    let dcdt_token_payment_one = DcdtTokenPayment::<StaticApi>::new(
        test_token_one_identifier.into(),
        0,
        BigUint::from(ONE_HUNDRED_THOUSAND),
    );

    let dcdt_token_payment_two = DcdtTokenPayment::<StaticApi>::new(
        test_token_two_identifier.into(),
        0,
        BigUint::from(ONE_HUNDRED_THOUSAND),
    );

    let payments_vec = PaymentsVec::from(vec![
        dcdt_token_payment_one.clone(),
        dcdt_token_payment_two.clone(),
    ]);

    state.deposit_with_logs(
        USER.to_managed_address(),
        OptionalValue::None,
        payments_vec.clone(),
        None,
        Some("deposit"),
    );

    state.common_setup.check_sc_dcdt_balance(
        vec![
            MultiValue3::from((test_token_one_identifier, 0u64, 0u64)),
            MultiValue3::from((test_token_two_identifier, 0u64, 0u64)),
        ],
        DCDT_SAFE_ADDRESS.to_managed_address(),
        sov_dcdt_safe::contract_obj,
    );

    let expected_amount_token_one =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_one.amount;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_one_identifier, &expected_amount_token_one);

    let expected_amount_token_two =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_two.amount;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_two_identifier, &expected_amount_token_two);
}

/// Test the deposit function with fee and without transfer data.
/// Steps:
/// 1. Deploy the Sov-DCDT-Safe smart contract with roles.
/// 2. Deploy the Fee-Market smart contract.
/// 3. Deploy the Testing smart contract.
/// 4. Set the Fee-Market address.
/// 5. Create a fee payment.
/// 6. Create two DCDT token payments.
/// 7. Create a payments vector with the fee payment and the two DCDT token payments.
/// 8. Call the deposit function with the payments vector.
/// 9. Check the DCDT balances of the addresses
#[test]
fn deposit_with_fee_no_transfer_data() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();

    let per_transfer = BigUint::from(100u64);
    let per_gas = BigUint::from(1u64);
    let fee_token_identifier = TestTokenIdentifier::new(FEE_TOKEN);

    let fee = FeeStruct {
        base_token: fee_token_identifier.into(),
        fee_type: FeeType::Fixed {
            token: fee_token_identifier.into(),
            per_transfer: per_transfer.clone(),
            per_gas: per_gas.clone(),
        },
    };

    state.common_setup.deploy_fee_market(Some(fee));
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    let test_token_one_identifier = TestTokenIdentifier::new(FIRST_TEST_TOKEN);
    let test_token_two_identifier = TestTokenIdentifier::new(SECOND_TEST_TOKEN);

    let fee_amount = BigUint::from(ONE_HUNDRED_THOUSAND);

    let fee_payment =
        DcdtTokenPayment::<StaticApi>::new(fee_token_identifier.into(), 0, fee_amount.clone());

    let dcdt_token_payment_one = DcdtTokenPayment::<StaticApi>::new(
        test_token_one_identifier.into(),
        0,
        BigUint::from(100u64),
    );

    let dcdt_token_payment_two = DcdtTokenPayment::<StaticApi>::new(
        test_token_two_identifier.into(),
        0,
        BigUint::from(100u64),
    );

    let payments_vec = PaymentsVec::from(vec![
        fee_payment,
        dcdt_token_payment_one.clone(),
        dcdt_token_payment_two.clone(),
    ]);

    state.deposit(
        USER.to_managed_address(),
        OptionalValue::None,
        payments_vec.clone(),
        None,
        Some("deposit"),
    );

    let expected_amount_token_one =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_one.amount;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_one_identifier, expected_amount_token_one);

    let expected_amount_token_two =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_two.amount;

    state.common_setup.check_sc_dcdt_balance(
        vec![
            MultiValue3::from((test_token_one_identifier, 0u64, 0u64)),
            MultiValue3::from((test_token_two_identifier, 0u64, 0u64)),
        ],
        DCDT_SAFE_ADDRESS.to_managed_address(),
        sov_dcdt_safe::contract_obj,
    );

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_two_identifier, expected_amount_token_two);

    let expected_amount_token_fee =
        BigUint::from(ONE_HUNDRED_MILLION) - BigUint::from(payments_vec.len() - 1) * per_transfer;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(fee_token_identifier, expected_amount_token_fee);
}

/// Test the deposit function without fee and with transfer data.
/// Steps:
/// 1. Deploy the Sov-DCDT-Safe smart contract with roles.
/// 2. Deploy the Fee-Market smart contract.
/// 3. Deploy the Testing smart contract.
/// 4. Set the Fee-Market address.
/// 5. Create two DCDT token payments.
/// 6. Create a payments vector with the two DCDT token payments.
/// 7. Call the deposit function with the payments vector.
/// 8. Check the logs for the deposit function.
/// 9. Check the DCDT balance of the addresses
#[test]
fn deposit_no_fee_with_transfer_data() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();

    state.common_setup.deploy_fee_market(None);
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    let test_token_one_identifier = TestTokenIdentifier::new(FIRST_TEST_TOKEN);
    let test_token_two_identifier = TestTokenIdentifier::new(SECOND_TEST_TOKEN);

    let dcdt_token_payment_one = DcdtTokenPayment::<StaticApi>::new(
        test_token_one_identifier.into(),
        0,
        BigUint::from(ONE_HUNDRED_THOUSAND),
    );

    let dcdt_token_payment_two = DcdtTokenPayment::<StaticApi>::new(
        test_token_two_identifier.into(),
        0,
        BigUint::from(ONE_HUNDRED_THOUSAND),
    );

    let payments_vec = PaymentsVec::from(vec![
        dcdt_token_payment_one.clone(),
        dcdt_token_payment_two.clone(),
    ]);

    let gas_limit = 1;
    let function = ManagedBuffer::<StaticApi>::from("hello");
    let args =
        MultiValueEncoded::<StaticApi, ManagedBuffer<StaticApi>>::from(ManagedVec::from(vec![
            ManagedBuffer::from("1"),
        ]));

    let transfer_data = MultiValue3::from((gas_limit, function, args));

    state.deposit_with_logs(
        USER.to_managed_address(),
        OptionalValue::Some(transfer_data),
        payments_vec.clone(),
        None,
        Some("deposit"),
    );

    let expected_amount_token_one =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_one.amount;

    state.common_setup.check_sc_dcdt_balance(
        vec![
            MultiValue3::from((test_token_one_identifier, 0u64, 0u64)),
            MultiValue3::from((test_token_two_identifier, 0u64, 0u64)),
        ],
        DCDT_SAFE_ADDRESS.to_managed_address(),
        sov_dcdt_safe::contract_obj,
    );

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_one_identifier, &expected_amount_token_one);

    let expected_amount_token_two =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_two.amount;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(
            TokenIdentifier::from(SECOND_TEST_TOKEN),
            &expected_amount_token_two,
        );
}

/// Test the deposit function with fee and with transfer data.
/// Steps:
/// 1. Deploy the Sov-DCDT-Safe smart contract with roles.
/// 2. Deploy the Fee-Market smart contract.
/// 3. Deploy the Testing smart contract.
/// 4. Set the Fee-Market address.
/// 5. Create a fee payment.
/// 6. Create two DCDT token payments.
/// 7. Create a payments vector with the fee payment and the two DCDT token payments.
/// 8. Call the deposit function with the payments vector.
/// 9. Check the DCDT balances of the addresses
#[test]
fn deposit_with_fee_with_transfer_data() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();

    let per_transfer = BigUint::from(100u64);
    let per_gas = BigUint::from(1u64);
    let fee_token_identifier = TestTokenIdentifier::new(FEE_TOKEN);

    let fee = FeeStruct {
        base_token: fee_token_identifier.into(),
        fee_type: FeeType::Fixed {
            token: fee_token_identifier.into(),
            per_transfer: per_transfer.clone(),
            per_gas: per_gas.clone(),
        },
    };

    state.common_setup.deploy_fee_market(Some(fee));
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    let test_token_one_identifier = TestTokenIdentifier::new(FIRST_TEST_TOKEN);
    let test_token_two_identifier = TestTokenIdentifier::new(SECOND_TEST_TOKEN);

    let fee_amount = BigUint::from(ONE_HUNDRED_THOUSAND);

    let fee_payment =
        DcdtTokenPayment::<StaticApi>::new(fee_token_identifier.into(), 0, fee_amount.clone());

    let dcdt_token_payment_one = DcdtTokenPayment::<StaticApi>::new(
        test_token_one_identifier.into(),
        0,
        BigUint::from(100u64),
    );

    let dcdt_token_payment_two = DcdtTokenPayment::<StaticApi>::new(
        test_token_two_identifier.into(),
        0,
        BigUint::from(100u64),
    );

    let payments_vec = PaymentsVec::from(vec![
        fee_payment,
        dcdt_token_payment_one.clone(),
        dcdt_token_payment_two.clone(),
    ]);

    let gas_limit = 2;
    let function = ManagedBuffer::<StaticApi>::from("hello");
    let args =
        MultiValueEncoded::<StaticApi, ManagedBuffer<StaticApi>>::from(ManagedVec::from(vec![
            ManagedBuffer::from("1"),
        ]));

    let transfer_data = MultiValue3::from((gas_limit, function, args));

    state.deposit(
        USER.to_managed_address(),
        OptionalValue::Some(transfer_data),
        payments_vec.clone(),
        None,
        Some("deposit"),
    );

    let expected_amount_token_one =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_one.amount;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(test_token_one_identifier, expected_amount_token_one);

    let expected_amount_token_two =
        BigUint::from(ONE_HUNDRED_MILLION) - &dcdt_token_payment_two.amount;

    state.common_setup.check_sc_dcdt_balance(
        vec![
            MultiValue3::from((test_token_one_identifier, 0u64, 0u64)),
            MultiValue3::from((test_token_two_identifier, 0u64, 0u64)),
        ],
        DCDT_SAFE_ADDRESS.to_managed_address(),
        sov_dcdt_safe::contract_obj,
    );

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(
            TokenIdentifier::from(SECOND_TEST_TOKEN),
            expected_amount_token_two,
        );

    let expected_amount_token_fee = BigUint::from(ONE_HUNDRED_MILLION)
        - BigUint::from(payments_vec.len() - 1) * per_transfer
        - BigUint::from(gas_limit) * per_gas;

    state
        .common_setup
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(fee_token_identifier, expected_amount_token_fee);
}

/// Test the deposit function with no transfer data and no payments
#[test]
fn deposit_no_transfer_data_no_payments() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();
    state.common_setup.deploy_fee_market(None);
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    state.deposit(
        USER.to_managed_address(),
        OptionalValue::None,
        PaymentsVec::new(),
        Some(NOTHING_TO_TRANSFER),
        None,
    );
}

/// Test the deposit function with only cross-chain sc call
/// Steps:
/// 1. Deploy the Sov-DCDT-Safe smart contract with roles.
/// 2. Deploy the Fee-Market smart contract.
/// 3. Deploy the Testing smart contract.
/// 5. Set the Fee-Market address.
/// 6. Create the cross-chain sc call TransferData
/// 7. Check for the `scCall` log
#[test]
fn deposit_sc_call_only() {
    let mut state = SovDcdtSafeTestState::new();

    state.deploy_contract_with_roles();
    state.common_setup.deploy_fee_market(None);
    state.common_setup.deploy_testing_sc();
    state.set_fee_market_address(FEE_MARKET_ADDRESS);

    let gas_limit = 2;
    let function = ManagedBuffer::<StaticApi>::from("hello");
    let args =
        MultiValueEncoded::<StaticApi, ManagedBuffer<StaticApi>>::from(ManagedVec::from(vec![
            ManagedBuffer::from("1"),
        ]));

    let transfer_data = MultiValue3::from((gas_limit, function, args));

    state.deposit(
        USER.to_managed_address(),
        OptionalValue::Some(transfer_data.clone()),
        PaymentsVec::new(),
        None,
        Some("scCall"),
    );
}
