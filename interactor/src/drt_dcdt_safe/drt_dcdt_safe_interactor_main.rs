...
use dharitri_sc_snippets::dharitri_sc_scenario::scenario_model::{Log, TxResponseStatus};
use dharitri_sc_snippets::sdk::gateway::SetStateAccount;
...
impl DrtDcdtSafeInteract {
    ...
    pub async fn deploy_drt_dcdt_safe(
        &mut self,
        ...
        self.state
            .set_drt_dcdt_safe_contract_address(Bech32Address::from_bech32_string(
                new_address_bech32.clone(),
            ));

        println!("new drt dcdt safe address: {new_address_bech32}");
    }
    ...
    pub async fn deploy_fee_market(
        &mut self,
        dcdt_safe_address: Bech32Address,
    ...
    pub async fn set_dcdt_safe_address_in_header_verifier(&mut self) {
        let response = self
            .interactor
            .tx()
            .from(&self.owner_address)
            ...
            .gas(90_000_000u64)
            .typed(HeaderverifierProxy)
            .set_dcdt_safe_address(self.state.current_drt_dcdt_safe_contract_address())
    ...
    pub async fn upgrade(&mut self) {
        let response = self
            .interactor
            .tx()
            .to(self.state.current_drt_dcdt_safe_contract_address())
            .from(&self.owner_address)
            .gas(90_000_000u64)
            .typed(DrtDcdtSafeProxy)
            .upgrade()
    ...
    pub async fn update_configuration(&mut self, new_config: DcdtSafeConfig<StaticApi>) {
        let response = self
            .interactor
            .tx()
            .from(&self.owner_address)
